	...
	var _pub_static = function () {
		...
		_main = function () {
			...
			//$(function () {
				...
				_pri.getIni(function (oResp) {
					var oIni = oResp.data;
					_pri["oIni"] = oIni;
                    oIni.fontCustomAble = (oIni.fontCustomAble && oIni.fontCustomName.trim());
		...
		_parseDOM = function () {
			...
			_pri.node = {
				...
				showTextFormat : JH.e('#showTextFormat'),
				fontCustom : JH.e('#fontCustom'),
				fontCustomName : JH.e('#fontCustomName'),
		...
		_uiEvt = function () {
			...
			JH.addEvent(_pri.node['showTextFormat'], 'click', _pri.uiEvtCallback.clickShowTextFormat);
			JH.addEvent(_pri.node['fontCustom'], 'click', _pri.uiEvtCallback.clickFontCustom);
			JH.addEvent(_pri.node['fontCustomName'], 'input', _pri.uiEvtCallback.inputFontCustomName);
		...
		//+++ 私有属性和方法定义区 +++++++++++++
            ...
            renderCustomUi: function (checked) {
                if (checked) {
                    $('#fontUse').hide();
                    $('#fontCustomName').show().focus();
                }else{
                    $('#fontUse').show();
                    $('#fontCustomName').hide();
                }
            ...
            resetFont: function () {
                let font = _pri.oIni.fontCustomAble ? _pri.oIni.fontCustomName : _pri.oIni.fontUse;
                $('head').append('<style type="text/css"> body,  button,  input,  textarea {font-family:'+font+';}</style>');
			...
			loadSetting : function (oIni) {
                ...
                _pri.node['fontCustom'].checked = oIni.fontCustomAble;
                _pri.renderCustomUi(_pri.node['fontCustom'].checked);
				_pri.node['sortKey'].checked = oIni.sortKey;
				_pri.node['fontCustomName'].value = oIni.fontCustomName;
				_pri.node['saveKeyStatus'].checked = oIni.saveKeyStatus;
				_pri.node['showTextFormat'].checked = oIni.showTextFormat;
			...
			uiEvtCallback : {
				clickShowTextFormat : function (evt) {
					_pri.saveSetting(this.name, this.value);
				},
				clickFontCustom : function () {
                    _pri.renderCustomUi(this.checked);
                    if (this.checked) {
                        _pri.saveSetting('fontUse', _pri.node.fontCustomName.value);
                    }
                    _pri.saveSetting('fontCustomName', _pri.node.fontCustomName.value);
                    _pri.saveSetting('fontCustomAble', this.checked);
                    _pri.resetFont();
				},
				inputFontCustomName : function (evt) {
                    _pri.saveSetting('fontCustomName', _pri.node.fontCustomName.value);
                    _pri.resetFont();
				},
				...
				clickShowTextFormat : function () {
					_pri.saveSetting('showTextFormat', this.checked);
				},
				changeLang : function () {

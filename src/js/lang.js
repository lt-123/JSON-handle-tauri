...
JH.mod.add([], 'lang', function (sModName, JH) {
	...
	_pri_s["data"] = {
		...
		'en' : {
			...
			'title_32' : 'Display format of text',
			'button_1' : 'Modify',
	...
	var _pub_static = function () {var _pub = {}, _pri = {}, _pro = {};
		...
		_pub["setPage"] = function (sLang) {
			...
			JH.forIn(_pri_s.data[sLang], function (o, k) {
				$('.langID_' + k, $p).html(_pub.getStr(k));
				$('.langTitleID_' + k, $p).attr('title', _pub.getStr(k));
				$('.langValueID_' + k, $p).attr('value', _pub.getStr(k));
			});
			$('.langID_data', $p).each(function () {
				try{
